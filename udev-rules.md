...
There is currently a bug with a kernel module that causes crackling audio. It is due to a faulty power saving mechanism and it is enough to drive you mad if you do not know what it is that is causing it. This rule changes the `snd_hda_intel` audio power-saving timeout depending on whether the system is connected to external power. Also this might be obvious to some, but **FYI:** Even though it has "Intel" in it's name it is not exclusive to computers with Intel hardware. To check if this is relevant to your system, run: `cat /sys/module/snd_hda_intel/parameters/power_save 2>/dev/null || echo "Not relevant"` - If it returns `Not Relevant` you may skip this rule. 

Why do it: `snd_hda_intel` can power down an idle audio codec to reduce energy use. However, waking the codec again may introduce a short delay or audible clicking and popping on some hardware. If you have this classic type of issue on Linux which is an audio issue then it will drive you nuts trying to fix it. This can be an issue on both desktops and laptops so one might wonder why we don't just turn it off, but the reason why this rule exists is because on laptops if this is turned off by default it will affect your battery life, so this rule provides a good compromise: keep the audio device awake for consistent responsiveness while plugged in, while retaining the system's previous power-saving timeout on battery.
