...
There is currently a bug with a kernel module that causes crackling audio. It is due to a faulty power saving mechanism and it is enough to drive you mad if you do not know what it is that is causing it. This rule changes the `snd_hda_intel` audio power-saving timeout depending on whether the system is connected to external power. Also this might be obvious to some, but don't feel bad if it wasn't: Even though it has "Intel" in it's name it is not exclusive to computers with Intel hardware. It's just a bad name for a kernel module. To truly check if this is relevant to your system, run: `cat /sys/module/snd_hda_intel/parameters/power_save 2>/dev/null || echo "Not relevant"` If it returns `Not Relevant` you may skip this rule. 
