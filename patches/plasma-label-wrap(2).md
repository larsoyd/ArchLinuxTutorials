...
and replaces it with a single constant `wrapMode` value. The default in this guide is `Text.WordWrap`, matching what you originally asked for. The other three values are available via a single variable change in the sync script. See **Pick a wrap mode** below for the actual deviation each one produces relative to upstream — three of the four are real changes, one of them is a no-op.

## Pick a wrap mode

The point of this patch is to deviate from upstream's `(maximumLineCount === 1) ? Text.NoWrap : Text.Wrap`. Three of the four available `wrapMode` values produce a real change; one of them is a no-op. Pick based on which deviation you actually want.

- `Text.NoWrap` — forces single-line elide regardless of your label-lines setting. Different from upstream when `maximumLineCount > 1`. Tradeoff: you lose the multi-line label feature entirely. Probably not what you want unless you really hate wrapping.
- `Text.WordWrap` — wraps only at whitespace word boundaries. Different from upstream by **letting long single tokens overflow the label box** instead of mid-word breaking them. Filenames like `IMG_20251104_long_screenshot_name.png` or `archive.tar.gz` are single "words" by Qt's definition (no spaces), so they will paint outside the icon cell. KDE contributor olib141 flagged this on r/kde and they are correct. Whether this is a problem or the actual point depends on you — if you prefer overflow over ugly mid-word breaks like `screenshot_2025_long_underscor` / `e_name.png`, this is the value you want. **This is the default in the script** because it matches your original ask.
- `Text.WrapAnywhere` — wraps at any character, ignoring word boundaries even when they exist. Different from upstream by breaking `Annual Report.pdf` as `Annual Re` / `port.pdf` instead of at the space. Never overflows. Use this if you care about no-overflow more than you care about word boundaries.
- `Text.Wrap` — prefers word boundaries, falls back to mid-word only when a single token is too long. **This is identical to what upstream already uses** in the multi-line branch. The only place it differs from the unpatched code is the `maximumLineCount === 1` branch, where wrapping cannot produce multiple visible lines anyway. So setting `MODE="Text.Wrap"` makes this patch a no-op for any practically observable behavior. Listed for completeness; not useful as the actual replacement value.

If you want zero overflow ever, use `Text.WrapAnywhere` and accept the ugly breaks on multi-word names. If you want clean word-boundary wrapping and accept overflow on long single tokens, use `Text.WordWrap` (the default). If you want upstream's behavior, you don't need this patch.
...
The ternary is dropped entirely. With `maximumLineCount === 1` the wrap mode no longer matters anyway (a one-line label cannot wrap), so collapsing the conditional is safe in both branches. The deviation from upstream is that long single-token filenames now overflow the label box instead of being mid-word broken — see **Pick a wrap mode** above for the rationale.
...
Create `~/.local/bin/plasma-folder-label-wrap-sync` with this exact content. Change the `MODE` value at the top if you want something other than `Text.WordWrap`. See the **Pick a wrap mode** section above — note that `MODE="Text.Wrap"` is a no-op against current upstream behavior.
...
# Default Text.WordWrap (long single tokens overflow; no mid-word breaks).
# Text.Wrap is a no-op against upstream. Text.WrapAnywhere never overflows
# but ignores word boundaries even on multi-word names.
MODE="Text.WordWrap"
...
```bash
grep -nF 'wrapMode: Text.WordWrap' \
  ~/.local/share/plasma/plasmoids/org.kde.desktopcontainment/contents/ui/FolderItemDelegate.qml
```

Replace `Text.WordWrap` with whichever mode you set. Note: `grep -F 'Text.Wrap'` would also match `Text.WrapAnywhere` since it is a substring; if you set `WrapAnywhere`, grep for the full string.
