...
For **ANY** computer you should also strongly consider using LUKS encryption for protection against offline access to your files. If you are wondering on how to do that, you may follow **this** tutorial (Currently writing it). – **Be advised** however, that LUKS protection does come with **performance trade offs** in regards to workload and gaming, at least according to [**this**](https://old.reddit.com/r/linux/comments/15wyukc/the_real_performance_impact_of_using_luks_disk/) highly detailed report. The report state that a regression of at leas **50%(!)** is standard. According to some comments YMMV depending on your hardware, but a regression seems to unfortunately be a guarantee. Make up your own mind if this is worth it for you, however for a device that will be accessible easily you should strongly consider using LUKS regardless of the trade offs mentioned or at least trying it and seeing if you notice any major performance hits for your workload.
