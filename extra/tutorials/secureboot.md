...
Secure Boot **ONLY** verifies the boot chain. *It does not* encrypt the SSD. What that means is if someone steals your drive then your data can be accessed on another system by chrooting into it.

For **ANY** computer you should also strongly consider using LUKS encryption for protection against offline access to your files. If you are wondering on how to do that, you may follow **this** tutorial (Currently writing it). – **Be advised** however, that unlike enabling SecureBoot (which is free), LUKS protection come with **significant trade offs to performance** in regards to various heavy workload and gaming. At least according to [**this**](https://old.reddit.com/r/linux/comments/15wyukc/the_real_performance_impact_of_using_luks_disk/) highly detailed report. 

The report state that a *minimum* of **50%(!)** performance regression is standard. But its worth noting that as some point out in the comments that the mileage may vary depending on your hardware. But that mileage only goes so far, a regression seems to unfortunately be a guarantee. Make up your own mind if this cost to performance is worth it for you, however if your device will be easily accessible to the public I strongly urge at least trying LUKS regardless of the trade offs mentioned.
